<template>
  <v-container>
    <v-row>
      <v-col cols="12" md="2">
        <v-text-field v-model="search" append-inner-icon="mdi-magnify" label="#factura, Proveedor " variant="underlined"
          color="primary" />
      </v-col>
      <v-col cols="12" md="3">
      ...
      <v-col cols="12" md="1">
        <v-btn color="primary" @click="getPurchases()">Filtrar</v-btn>
      </v-col>
      <v-col cols="12" md="1">
          ...
          <v-container>
            <v-row>
              <v-col cols="12" md="6" class="text-left">
                ...
                <p><small><strong>Fecha</strong> {{ formatFecha(purchaseDetail.created_at) }}</small></p>
                <p>
              ...
              <v-col cols="12" md="4">
                <v-select :items="['Pagado', 'Pendiente de Pago']" v-model="purchaseDetail.status" label="Estado"
...
const getPurchases = async (page = 1) => {
  try {
    const res = await axiosInst.get(url + `api/purchase`, {
      params: {
        page,
        search: search.value || '',
