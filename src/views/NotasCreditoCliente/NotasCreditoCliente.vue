                        ...
                        <v-row align="center">
                            <v-col cols="12" md="8">
                                <v-text-field v-model="txtregdata.factura_numero" label="Número o código de factura"
                                    ...
                                    prepend-inner-icon="mdi-file-search-outline" hide-details
                                    @keyup.enter="searchCodigo" />
                            </v-col>
                            <v-col cols="12" md="4">
                        ...
                        <v-card class="rounded-xl elevation-4 mb-6 overflow-hidden">
                            <v-toolbar color="grey-darken-3" dark flat>
                                ...
                                <v-toolbar-title class="text-subtitle-1 font-weight-bold">
                                    Documento Original: {{ datosVenta.factura_numero }}
                                </v-toolbar-title>
                                <v-spacer></v-spacer>
                            </v-toolbar>
                            ...
                            <v-card-text class="pa-6">
                                <v-row>
                                    <v-col cols="12" md="4" class="border-right">
                                        ...
                                        <div class="text-subtitle-1 font-weight-black">{{ datosVenta.cliente }}</div>
                                    </v-col>
                                    <v-col cols="12" md="4" class="border-right">
                                        <div class="text-caption text-grey">Vendedor</div>
                                        <div class="text-subtitle-1 font-weight-black">{{ datosVenta.vendedor }}</div>
                                    </v-col>
                                    <v-col cols="12" md="4" class="border-right">
                                        <div class="text-caption text-grey">Fecha de Venta</div>
                                        <div class="text-subtitle-1 font-weight-medium">
                                            {{ new Date(datosVenta?.order_date).toLocaleString('es-CO') }}
                                        </div>
                                    </v-col>
                                    <v-col cols="12" md="4">
                                        <div class="text-caption text-grey">Condición de Pago</div>
                                        <div class="text-subtitle-1 font-weight-medium text-uppercase">{{
                                            datosVenta.forma_pago_nombre }} - {{ datosVenta.metodo_pago_nombre }}</div>
                                    </v-col>
                                </v-row>
                        ...
                        <v-card class="rounded-xl elevation-8 border-left-nota overflow-hidden">
                            ...
                            <v-card-text class="pa-6">
                                <v-row>
                                    <v-col cols="12" md="4">
                                        <v-toolbar-title class="text-subtitle-1 font-weight-bold">
                                            Documento Referencia: {{ datosVenta.factura_numero }}
                                        </v-toolbar-title>
                                    </v-col>
                                    <v-col cols="12" md="4">
                                        <div class="text-caption text-grey">Cliente :{{ datosVenta.cliente }}</div>
                                    </v-col>
                                    <v-col cols="12" md="4">
                                        <v-text-field v-model="datosNota.fecha_emision" type="date"
                                            label="Fecha Emisión" variant="underlined"></v-text-field>
                                    </v-col>
                                </v-row>
                                <v-row>
                                    <br>
                                    <v-col cols="12" md="4">
                                        <v-select v-model="datosNota.customization_id" :items="tipoNotas"
                                            label="Tipo de Nota (DIAN)" item-title="title" item-value="value"
                                            variant="outlined"
                                            @update:model-value="val => datosNota.tipo = (val === 20 ? 'credito' : 'debito')" />
                                    </v-col>

                                    <v-col cols="12" md="4">
                                        <v-select v-model="datosNota.correction_concept_code"
                                            :items="motivosNotaCredito" label="Concepto corrección" item-title="title"
                                            item-value="value" variant="outlined" />
                                    </v-col>
                                    <v-col cols="12" md="4">
                                        <v-select v-model="datosNota.payment_method_code" :items="metodosPago"
                                            label="Método de Compensación" item-title="title" item-value="value"
                                            variant="outlined" />
                                    </v-col>
                                </v-row>
                                <v-row>
                                    <v-col cols="12" md="6">
                                        <v-text-field v-model="datosNota.motivo" label="Observación"
                                ...
                                <v-table v-if="itemselected.length > 0" class="border rounded-lg shadow-inner">
                                    <thead class="bg-indigo-lighten-5">
                                        <tr>
                                            ...
                                            <th class="text-right">Entra a Stock</th>
                                        ...
                                        <tr v-for="item in itemselected" :key="item.id">
                                            ...
                                            <td class="text-center">
                                                <v-text-field v-model.number="item.cantidad" type="number"
                                                    density="compact" hide-details variant="solo" flat
                                                    bg-color="grey-lighten-4" class="centered-input"
                                                    :max="item.cantidad_maxima" min="1"
                                                    @input="validarCantidad(item)"></v-text-field>
                                                <div class="text-caption text-error font-weight-bold mt-1">
                                                    Máx: {{ item.cantidad_maxima }}
                                                </div>
                                            </td>
                                            <td class="text-right">{{ formato(item.precio_venta) }}</td>
                                            <td class="text-right">{{ item.iva }}%</td>
                                            <td class="text-right font-weight-bold">
                                                ${{ ((Number(item.cantidad) * Number(item.precio_venta)) * (1 +
                                                    (Number(item.iva ?? 0) / 100))).toLocaleString('es-ES', {
                                                        minimumFractionDigits: 2, maximumFractionDigits: 2
                                                    })
                                                }}
                                            </td>
                                            <td class="text-center">
                                                <v-checkbox label="Entra" v-model="item.entra_stock"
                                                    color="primary"></v-checkbox>
                                            </td>
                                            <td class="text-center">
                                                <v-btn icon="mdi-close-circle" color="error" variant="text"
                                ...
                                <v-alert v-else type="info" variant="tonal"
                                    text="No has seleccionado productos. Usa el icono (+) en la factura original."
                                    class="mt-4"></v-alert>
                                ...
                                <div class="d-flex justify-end align-center">
                                    ...
                                    <v-btn color="success" size="x-large" block :disabled="!notaEsValida"
                                        @click="crearNota()">
...
const searchCodigo = async () => {
    ...
    spinner.value = true
    try {
        const res = await axiosInst.get(`${url}api/ventasearch/${txtregdata.value.factura_numero}`)
        console.log(res.data)
        if (!res.data) {
            throw new Error('La respuesta del servidor está vacía');
        }

        const venta = res.data; // <--- Inicialización clara

        // Ahora asignamos a los refs usando la constante 'venta'
        datosVenta.value = {
            id: venta.id,
            factura_numero: venta.factura_numero,
            order_date: venta.created_at,
            forma_pago_nombre: venta.forma_pago_nombre,
            metodo_pago_nombre: venta.metodo_pago_nombre,
            total: venta.total,
            vendedor: venta.user ? `${venta.user.nombres} ${venta.user.apellidos} ${venta.user.numidentificacion || ''}`.trim() : 'Cliente no registrado',
            cliente: venta.client ? `${venta.client.nombres} ${venta.client.apellidos} ${venta.client.numidentificacion || ''}`.trim() : 'Cliente no registrado'
        };

        // Actualizamos datosNota con los campos técnicos para Factus
        datosNota.value.cliente_id = venta.cliente_id;
        datosNota.value.sale_id = venta.id;
        datosNota.value.bill_id_factus = venta.bill_id_factus || ''; // Campo nuevo de la migración

        // Mapeamos los productos
        itemsFactura.value = (venta.details || []).map((det) => ({
            id: det.producto_id,
...
const crearNota = async () => {
    ...
    try {
        ...
        const res = await axiosInst.post(url + 'api/notasventa', payload)
        snackbarReg.value = true
...
watch(itemselected, (nuevaLista) => {
    nuevaLista.forEach((item) => {
        ...
        item.ganancia = (ventaUsuario - venta).toFixed(2);
    });
}, { deep: true });
