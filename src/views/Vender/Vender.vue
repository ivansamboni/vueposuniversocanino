<template>
    ...
    <v-container fluid class="bg-grey-lighten-4 main-pos-container pa-0">
      <v-row class="fill-height-custom" no-gutters>
        <v-col cols="12" class="d-flex flex-column h-100">
                ...
                <v-row dense align="center">
                  ...
                  <v-col cols="12" md="2">
                    <v-text-field
                      ...
                      v-model="txtregdata.codigo_barras"
                      label="Código (Ctrl + C)"
                      variant="outlined"
                      color="primary"
                      density="compact"
                  ...
                  <v-col cols="12" md="2">
                    <v-combobox
                      ...
                      item-title="nombre"
                      item-value="codigo_barras"
                      v-model:search="nomBuscar"
                  ...
                  <v-col cols="12" md="2" class="d-flex justify-center">
                    <v-chip
                      :color="cajaMensajeEstado.status"
                      variant="flat"
                      size="small"
          ...
          <v-card
            elevation="4"
            class="rounded-lg overflow-hidden d-flex flex-column flex-grow-1 mb-4"
          >
            <!-- Tabla de productos -->
              ...
              <table fixed-header hover class="table">
                <thead>
                  <tr class="bg-grey-lighten-4">
                    ...
                    <th class="text-right text-uppercase text-caption font-weight-bold">
                      Precio Unit.
                    </th>
                    <th class="text-right text-uppercase text-caption font-weight-bold">
                      Subtotal
                ...
                <tbody>
                  <tr v-for="(item, index) in itemsVenta" :key="item.id">
                    ...
                    <td class="text-right">
                      <small
                        >${{ parseFloat(item.precio_final).toLocaleString("es-ES") }}</small
                      >
                    </td>
                    <td class="text-right font-weight-bold">
                      <small
                        >${{
                          (item.cantidad * item.precio_final).toLocaleString("es-ES")
                        }}</small
                      >
                    </td>
                    <td class="text-center">
            ...
            >
              <div class="d-flex flex-no-wrap align-center pa-3">
                <div style="width: 25%" class="border-e pr-4">
                  <v-chip
                    :color="dataPlan.is_active ? 'success' : 'error'"
                    size="x-small"
                    variant="flat"
                    class="mb-1"
                  >
                ...
                <div style="width: 30%" class="px-4 border-e">
                  <div class="d-flex align-center mb-1">
                    ...
                    <span class="text-body-2"
                      >{{ dataPlan.days_per_week }} días/semana</span
                    >
                  </div>
                  <div class="d-flex align-center">
                    ...
                    ></v-text-field>
                    <span class="text-body-2"
                      >Vence en: {{ custom_days }} días</span
                    >
                  </div>
                </div>
                ...
                <div style="flex-grow: 1" class="px-4">
                  ...
                  <div class="text-caption d-flex justify-space-between mt-1">
                    ...
                    <span class="text-primary"
                      ><strong>Mascota:</strong> {{ datosVenta.petname }}</span
                    >
                  </div>
                </div>
                ...
                <div style="width: 20%" class="text-right pl-4">
                  ...
                  <div class="text-h5 font-weight-bold text-success">          
                    ${{ parseFloat( precioPlanFinal).toLocaleString("es-ES") }}
                    <v-btn
                      icon="mdi-close-circle-outline"
                      variant="text"
                      color="error"
              ...
              <v-row align="center" no-gutters>
                ...
                <v-col cols="12" md="6">
                  <v-row dense align="center" class="pe-4">
                    ...
                    <v-col cols="4">
                      <v-select
                        v-if="datosVenta.forma_pago === '1'"
                        v-model="datosVenta.metodo_pago"
                        :items="metodosPago"
                        label="Método"
                        variant="outlined"
                        density="compact"
                    ...
                    <v-col cols="3">
                      <v-text-field
                        v-if="datosVenta.metodo_pago === '10'"
                        v-model="efectivoRecibidoFormatted"
                        label="Efectivo Recibido"
                        color="primary"
                        variant="outlined"
                        density="compact"
                        prefix="$"
                        @input="updateEfectivoRecibido"
                        hide-details
                      />
                    </v-col>
                  </v-row>
                ...
                <v-col cols="12" md="2" class="d-flex align-center justify-end ps-4">
                  <v-switch
                    v-model="activeImpresion"
                    density="compact"
                    hide-details
                    :label="activeImpresion ? 'Imprimir' : 'No Imprimir'"
                    color="primary"
                    class="me-6"
                  />
                </v-col>
                ...
                <v-col cols="12" md="2" class="text-center border-s border-e px-2">
                  <div class="d-flex flex-column align-center justify-center">
                    <span class="text-overline text-grey-darken-1" style="line-height: 1"
                      >TOTAL A PAGAR</span
                    >
                    <span
                      class="text-h4 font-weight-black text-primary-darken-1"
      ...
      <v-card class="rounded-xl">
        ...
        <v-card-text class="pa-6">
          <v-form>
            <v-row align="center">
              <v-col cols="12" md="8">
                <v-combobox
                  ...
                  v-model:search="buscarTexto"
                  :items="clientResults"
                  item-title="nombre"
                  item-value="id"
                  variant="outlined"
                  label="Buscar cliente por nombre o CC"
                  prepend-inner-icon="mdi-magnify"
                  color="primary"
                  hide-details
                  rounded="lg"
              ...
              <v-row dense>
                <v-col cols="12" md="7">
                  ...
                  <div class="d-flex align-center mb-2">
                    ...
                    <span class="ms-2 text-body-2">{{
                      clienteSeleccionado?.numidentificacion || "-"
                    }}</span>
                  </div>
                </v-col>
                <v-col cols="12" md="5">
                  ...
                  <div class="d-flex align-center mb-2">
                    ...
                    <span class="ms-2 text-body-2 text-truncate">{{
                ...
                <v-col cols="12">
                  ...
                  <div class="d-flex align-start">
                    <v-icon size="18" color="primary" class="me-2 mt-1"
            ...
            <v-alert
              ...
              icon="mdi-send-check"
            >
              La factura se enviará al correo
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn
            ...
            @click="
              clienteFinal(clienteSeleccionado.numidentificacion);
              dialogEdit = false;
            "
          >
        ...
        <v-card color="grey-lighten-4">
          ...
          <v-card-text class="pa-6">
            <v-row dense>
              ...
              >
                <v-card
                  class="mx-auto h-100 d-flex flex-column rounded-xl transition-swing"
                  elevation="1"
                  hover
                  border
                >
                  <div class="pa-3 d-flex justify-end">
                    <v-chip
                      :color="mac.is_active ? 'success' : 'error'"
                      size="x-small"
                      variant="tonal"
                  ...
                  <v-card-item class="pt-0 text-center">
                    <div class="text-h6 font-weight-black text-secondary mb-1">
                      {{ mac.name }}
                    </div>
        ...
        <v-card class="rounded-xl pa-2">
          ...
          <v-card-text>
            <v-list-item class="bg-blue-lighten-5 rounded-lg mb-4 pa-3">
              ...
              <v-list-item-subtitle
                >CC: {{ datosVenta.numidentificacion }}</v-list-item-subtitle
              >
            </v-list-item>
  
            <p class="text-caption text-grey-darken-1 mb-2 ml-1">VINCULAR A UNA MASCOTA:</p>
            <v-select
              v-model="datosVenta.pet_id"
              :items="pets"
              item-title="name"
              item-value="id"
              ...
              color="primary"
              rounded="lg"
              :hint="getPetHint(datosVenta.pet_id)"
              persistent-hint
              no-data-text="El cliente no tiene mascotas registradas"
          ...
          <v-card-actions class="pb-4 px-6">
            <v-btn variant="text" color="grey" @click="(dialog2 = false), cancelPlan()"
              >Cancelar</v-btn
            >
            <v-spacer />
            <v-btn
    ...
    <v-dialog max-width="500px" v-model="dialogImp">
      <v-card>
        <v-card-title class="justify-space-between">
          ...
          <v-btn
            icon="mdi-close"
            @click="(dialogImp = false), codigoBarrasInput?.value?.focus?.()"
          />
        </v-card-title>
        <v-divider />
        <v-card-text>
  ...
  const precioPlanFinal = computed(() => {
    // Si no hay días seleccionados o son iguales a la duración original del plan
    if (!custom_days.value || custom_days.value === dataPlan.value.duration_days) {
      custom_days.value = dataPlan.value.duration_days;
      return dataPlan.value.price;
    }
  
    const precioPorDia = dataPlan.value.price / dataPlan.value.duration_days;
    const subtotal = precioPorDia * custom_days.value
  
    const resultadoRedondeado = Math.round(subtotal / 100) * 100;
  ...
  const total = computed(() => {
    const totalProductos = itemsVenta.value.reduce(
      (acc, p) => acc + p.precio_final * p.cantidad,
      0
    );
  
    return totalProductos + precioPlanFinal.value;
  });
  
  const totalDevolver = computed(() => efectivoRecibido.value - total.value);
  ...
  const isDisabled = computed(() => {
    ...
    if (tieneProductos) {
      const cantidadesInvalidas = itemsVenta.value.some(
        (item) => item.cantidad === null || Number(item.cantidad) <= 0
      );
  ...
  const searchCodigo = async () => {
    try {
      const res = await axiosInst.get(
        `${url}api/searchcodigoproducto/${txtregdata.value.codigo_barras}`
      );
  
      if (res.data.stock === 0) {
        alert(
          "Alerta: Producto agotado en el sistema. Actualice stock en la opción de ajustes."
        );
      }
  
      const index = itemsVenta.value.findIndex(
        (item) =>
          item.id === res.data.id || item.codigo_barras === txtregdata.value.codigo_barras
      );
  ...
  const finishSale = async () => {
    ...
    try {
      const payload = {
        ...
        custom_days:custom_days.value,
        custom_price_plan:precioPlanFinal.value,
