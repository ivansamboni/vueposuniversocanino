      ...
      <v-col cols="12" md="1">
        <v-dialog width="2000px" v-model="modalNueva">
          ...
          <template v-slot:default="{ isActive }">
            <v-card title="Devoluciones a Proveedor">
              <template v-slot:text>
                <NotasCreditoProveedor @loadNotas="getNotas()" />
              </template>
              <v-card-actions>
                <v-spacer></v-spacer>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr v-for="ord in notasList.data" :key="ord.id">
        ...
        <td><small>{{ ord.purchase.seller.nombres ?? '' }} , {{ ord.purchase.seller.apellidos ?? '' }}</small></td>
        <td><small>{{ ord.referencia_proveedor }}</small></td>
  ...
  <v-dialog v-model="dialogEdit" max-width="850">
    <v-card class="rounded-xl elevation-24 bg-grey-lighten-5">
      <v-toolbar color="white" flat class="border-b px-4">
        <v-icon color="primary" class="mr-3">mdi-file-document-edit-outline</v-icon>
      ...
      <v-card-text class="pa-6 pa-md-10 bg-white">
        <v-row class="mb-6" align="start">
          <v-col cols="12" sm="6">
            <div class="text-overline text-primary font-weight-bold mb-1">Nota Crédito N°</div>
            <div class="text-h4 font-weight-black mb-1">{{ notasDetail.referencia_proveedor }}</div>

            <div class="d-flex align-center mt-2">
              <v-icon size="small" color="grey-darken-2" class="mr-1">mdi-link-variant</v-icon>
              <span class="text-caption text-grey-darken-2">
                Afecta a Factura: <strong>{{ notasDetail.purchase?.factura_numero
                  || 'No asociada' }}</strong>
              </span>
            </div>

            <v-chip size="small" color="blue-grey-lighten-5" class="mt-2 font-weight-medium">
              Emitido: {{ notasDetail.fecha_emision }}
            </v-chip>
            </v-col>

            <v-col cols="12" sm="6" class="text-sm-right">
              <div class="text-overline text-grey-darken-1 mb-1">Proveedor</div>
              <div class="text-h6 font-weight-bold leading-tight">
                {{ notasDetail.purchase?.seller?.nombres }} {{ notasDetail.purchase?.seller?.apellidos }}
              </div>
              <div class="text-body-2 text-grey-darken-1">
                {{ notasDetail.purchase?.seller?.tipoidentificacion }}: {{
                  notasDetail.purchase?.seller?.numidentificacion
                }}
              </div>
              <v-chip size="x-small" color="grey-darken-4" variant="flat" class="mt-2">
        ...
        <div class="text-subtitle-2 font-weight-bold mb-3 ml-1 text-grey-darken-3">Detalle de Artículos</div>
        <v-table class="mb-6 border rounded-lg">
          ...
          <tbody>
            <tr v-for="detalle in notasDetail.notadetails" :key="detalle.id">
              <td class="py-3">
                <div class="font-weight-bold text-body-2">{{ detalle.producto.nombre }}</div>
                <div class="text-caption text-grey font-italic">{{ detalle.producto.codigo_barras }}</div>
              </td>
        ...
        <v-row>
          <v-col cols="12" md="7">
            <v-sheet border rounded="lg" class="pa-4 bg-yellow-lighten-5" v-if="notasDetail.motivo">
              <div class="text-caption font-weight-bold text-grey-darken-3 mb-1">MOTIVO / OBSERVACIONES:</div>
          ...
          <v-col cols="12" md="5">
            <div class="d-flex justify-space-between mb-2">
              <span class="text-grey-darken-1">Subtotal Neto:</span>
              <span class="font-weight-medium">${{ (Number(notasDetail.total) -
                calcularTotalIva(notasDetail.notadetails)).toLocaleString('es-CO') }}</span>
            </div>
            <div class="d-flex justify-space-between mb-2">
              <span class="text-grey-darken-1">Impuestos (IVA):</span>
            ...
            <div class="text-right text-caption text-grey-darken-1 mt-1">Valores expresados en Pesos Colombianos</div>
          </v-col>
        </v-row>
      </v-card-text>
      ...
      <v-card-actions class="pa-6 bg-grey-lighten-4">
        ...
        <v-spacer></v-spacer>
        <v-btn color="grey-darken-4" variant="outlined" prepend-icon="mdi-download" class="mr-2 text-none">
          Descargar PDF
        </v-btn>
        <v-btn color="primary" variant="flat" prepend-icon="mdi-printer" class="px-8 text-none font-weight-bold">
          Imprimir Nota
        </v-btn>
      </v-card-actions>
    </v-card>
...
import NotasCreditoProveedor from '@/views/NotasCreditoDebito/NotasCreditoProveedor.vue'
...
const getNotas = async (page = 1) => {
  ...
  try {
    const res = await axiosInst.get(url + `api/notas`, {
      params: {
        page,
        search: search.value || '',
...
const verOrden = async (id) => {
  ...
  const res = await axiosInst.get(url + 'api/notas/' + id);
